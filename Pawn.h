...
*  Pawn:
     ...
     Actions: initalizes a Pawn object by setting the colour to White or black
     Assumptions: none

*  getSymbol:
     Input: none
     Actions: returns the char symbol of the current piece
     Assumptions: Pawn has been initialized

*  canMove:
     Input: current board object, old and new x-coordinates, old and new 
            y-coordinates
     Actions: checks if a pawn on the board is allowed to move from it's
              current position into the new position proposed. This is all
              of course depending on the rules of chess. 
     Assumptions: Pawn and Board objects are initialized

*  clone:                                                                      
     Input: None                                                               
     Actions: duplicates the current Pawn object and returns a pointer to    
              the new one                                                      
     Assumptions: Pawn is initilaized    

*  notifyMoved:
     Input: nothing
     Actions: sets hasMoved to be equal to true
     Assumptions: Pawn is initalized 
*/
#ifndef PAWN_H
#define PAWN_H
