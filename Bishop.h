...
*  Bishop:
     ...
     Actions: initalizes a Bishop object by setting the colour to White or 
              black
     Assumptions: none
...
*  getSymbol:
     ...
     Assumptions: Bishop has been initialized
                                                
*  canMove:
     Input: current board object, old and new x-coordinates, old and new 
            y-coordinates
     Actions: checks if a Bishop on the board is allowed to move from it's
              current position into the new position proposed. This is all
              of course depending on the rules of chess. 
     Assumptions: Bishop and Board objects are initialized

*  clone:
     Input: None
     Actions: duplicates the current Bishop object and returns a pointer to
              the new one
     Assumptions: Bishop is initilaized 
*/
#ifndef BISHOP_H
#define BISHOP_H
